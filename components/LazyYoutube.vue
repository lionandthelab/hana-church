<template>
  <VideoWrapper :aspectRatioValue="aspectRatioValue" :maxWidth="maxWidth">
    <Preview v-if="!onceLoaded" @click="createIframe(isVideoFound, 'youtube')">
      <template v-if="isVideoFound">
        <img
          ...
          alt=""
          @error="
            $event.target.src = getYoutubeThumbnail(videoID, thumbnailQuality)
          "
        />
        <img
          v-else
          :src="getYoutubeThumbnail(videoID, thumbnailQuality)"
          alt=""
        />
        <template v-if="showTitle">
          <span class="ly-text">{{
            isCustomTitleExist ? customTitle : getTitle
          }}</span>
        </template>
    ...
    <div class="ly-iframe-wrapper" v-show="onceLoaded"></div>
  </VideoWrapper>
</template>
